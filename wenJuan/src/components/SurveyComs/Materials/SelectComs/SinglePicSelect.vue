<template>
  ...
  >
    ...
    <el-radio-group>
      <el-radio
        v-for="(option, index) in computedStatus.options"
        :value="option.picTitle"
        :key="index"
        class="picOption flex mb-15"
      >
      <PicItem :key="index" v-bind="{ ...option, index }" />
      </el-radio>
    </el-radio-group>
  </div>
</template>
...
import {
  getTextStatus,
  getPicTitleDescStatus,
  getCurrentStatus,
  getStringStatusByCurrentIndex,
} from '@/utils';
...
const computedStatus = computed(() => ({
  ...
  options: getPicTitleDescStatus(props.status.options),
